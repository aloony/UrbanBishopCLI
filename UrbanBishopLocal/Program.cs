...
namespace UrbanBishopLocal
{
    class Program
    {
        ...
        static void Runner(byte[] data, char[] key)
        {
        ...
        static void ValidateCommandLineArguments(string[] args) {
            ...
            char[] key = {};
                ...
                if(args.Length == 0)
                {
                    Console.WriteLine("Pass at least one argument");  
                    return;          
                }
                else if (args.Contains("--help") || args.Contains("-h")){
                    Console.WriteLine("havent done yet, sry im kinda busy rn, fork it if you r fine with fixing that :)");
                    return;}

                else if (args.Contains("--path") || args.Contains("-p")){
                    b64shellcode = File.ReadAllText(args[Array.FindIndex(args, s => s == "--path" || s == "-p")+1]);}

                else if (args.Contains("--key") || args.Contains("-k")){
                    key = new char[args[Array.FindIndex(args, s => s == "--key" || s == "-k")+1].Length]; 
                    for (int i = 0; i < args[Array.FindIndex(args, s => s == "--key" || s == "-k")+1].Length; i++) { 
                    key[i] = args[Array.FindIndex(args, s => s == "--key" || s == "-k")+1][i]; }
                } else {
                    Console.WriteLine("You've passed a wrong argument");  
                    return; 
                }     

            Runner(Convert.FromBase64String(b64shellcode), key);
        }
